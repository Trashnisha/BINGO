function BingoCard(){
    ...
    this.generateMatrix=()=>{
        ...
        for(var a=0;a<5;a++){
            g[a]=Math.floor(Math.random()*(60-46)+1)
        }
        for(var outer=0; outer<g.length;outer++){
            for(var inner=0;inner<g.length;inner++){
                if(inner!==outer & g[outer]==[inner]){
                    g[outer]=Math.floor(Math.random()*(60-46)+1)
                }
            }
        }
        this.matrix.push(g)
